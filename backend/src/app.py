...
def api_predict_beds():
    ...
    Uses the LightGBM model to estimate future bed availability.
    """
    try:
        hospitals = get_all_hospitals()
        results = []
        for h in hospitals:
            ...
            # Run LightGBM congestion prediction
            congestion_score = predict_congestion(h)
            # Derive predicted available beds:
            # Higher congestion → fewer beds. Score maps to occupancy change.
            max_score = 300  # rough max from training formula
            congestion_pct = min(1.0, max(0.0, congestion_score / max_score))
            occupied_predicted = round(total_beds * congestion_pct)
            predicted_beds = max(0, total_beds - occupied_predicted)
            results.append({
        ...
        logger.error(f"Error predicting beds: {e}")
        return jsonify({"error": str(e)}), 500
